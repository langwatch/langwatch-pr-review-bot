...
- The PR diff is **not capped**. The whole elided diff is handed to the reviewer, which reads it agentically across as many turns as the PR needs (per-file `Read` with offset/limit, `Grep`), so a large PR gets full coverage. A 512 KiB byte budget was removed: it rested on the false premise that one review pass must fit the diff into a single read — it does not, and the real API request-size limit sits far above it. The review body carries no truncation notice.
