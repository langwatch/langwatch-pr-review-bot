...
- Files marked `linguist-generated=true` in the `.gitattributes` **on the base branch** are elided from the review diff before the byte budget is applied, so large generated blobs (prisma manifests, big fixtures) do not crowd out human-authored files. Attributes are read from the base ref, not the PR head, so a pull request cannot hide its own files by marking them generated. The review body and reviewer prompt report the elided file count. To opt a file in, add a line such as `path/to/file.generated.json linguist-generated=true` to the base branch's `.gitattributes`; the literal value `true` is required (`set` or any other value keeps the file in the diff). Resolving attributes on the base ref needs git ≥ 2.40 on the runner; the action fails loudly if git is older.
...
<!-- elision proof PR: throwaway, do not merge -->
